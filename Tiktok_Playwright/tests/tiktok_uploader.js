...
(async () =>{
    ...
    await context.addCookies([COOKIES HERE]);
  ...
  await page.dispatchEvent(selector, 'drop', { dataTransfer });
    ...
    if (args.length < 2) {
        console.log('Please provide at least two arguments: <arg1> <arg2> (or) => <videoNumber> <date YYYY-MM-DD> <productName> <location>');
        process.exit(1); // Exit with an error code
    }

    // Use the arguments
    const videoNumber = args[0];
    const date = args[1];
    let productName = 'Shaping Butt Lifter Bodysuit';
    let location = 'Miami';
    //if args[2] not defined set productName to default value
    if(args[2]){
